...
// Изменение задачи
app.put('/tasklists/:tasklistId', async (request, response) => {
  ...
  tasklists[tasklistId].tasklistName = newTaskName;
  await writeData(tasklists);
  response
    .setHeader('Content-Type', 'application/json')
    .status(200)
    .json({
      info: `Task №${tasklistId} was successfully edited`
    });
});
...
app.delete('/tasklists/:tasklistId', async (request, response) => {
  ...
  const deletedTaskName = tasklists[tasklistId];
  ...
  response
    ...
    .status(200)
    .json({
      info: `Task '${deletedTaskName}' was successfully deleted`
    });
});
...
app.post('/tasklists', async (request, response) => {
  const { tasklistName, counter } = request.body;
...
app.listen(port, hostname, async (err) => {
  ...
  tasklistsFromFile.forEach(({ tasklistName, tasks, counter }) => {
    tasklists.push({
